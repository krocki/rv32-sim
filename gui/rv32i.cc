// rv32i_sim.cpp — RV32I simulator with optional Dear ImGui GUI and step-back history
// -----------------------------------------------------------------------------
// CLI-only build:
//   g++ -std=c++17 -O2 rv32i_sim.cpp -o rv32i_sim
// GUI build (needs SDL2 + OpenGL3 + Dear ImGui):
//   g++ -std=c++17 -O2 -DGUI -lSDL2 -lGL -ldl -pthread rv32i_sim.cpp -o rv32i_gui
// (Ensure imgui headers + back-ends are reachable via -I flags or your include path.)
...
// ────────────────────────────────────────────────────────────────────────────
// CPU core (headless) — all GUI/CLI code reuses this
// ─────────────────────────────────────────────────────────────────────────────
struct CPU {
  struct Snapshot {                     // minimal state for step-back
    uint32_t pc; std::array<uint32_t,32> x; uint64_t cycles;
  };
  ...
  // execution trace for GUI history -----------------------------------------
  struct Trace { uint64_t cyc; uint32_t pc, ins; std::string txt; };
  std::deque<Trace> trace;
  size_t trace_max = 10000;

  std::deque<Snapshot> history;        // for step-back
  size_t history_max = 512;            // configurable
  ...
  void reset() { pc = cycles = 0; x.fill(0); clear_changes(); history.clear(); trace.clear(); }
  ...
  // — helpers — -------------------------------------------------------------
  static uint32_t sx(uint32_t v, int bits) { uint32_t m = 1u << (bits - 1); return (v ^ m) - m; }
  ...
  void store32(uint32_t a, uint32_t v){ assert(a+3<mem.size()); mem[a]=v&0xFF; mem[a+1]=v>>8; mem[a+2]=v>>16; mem[a+3]=v>>24; }

  // simple RV32I disassembler (name + operands) -----------------------------
  std::string disasm(uint32_t ins) const {
    ...
    auto imm_j=[&]{ uint32_t v=((ins>>21)&0x3FF)<<1; v|=((ins>>20)&1)<<11; v|=((ins>>12)&0xFF)<<12; v|=(ins>>31)<<20; return sx(v,21);} ;

    std::ostringstream os; os << std::dec;
    switch(opc){
      ...
      case 0x0F: os<<"fence"; break; case 0x73: os<<"ecall"; break; default: os<<"illegal"; break;
    }
    return os.str();
  }
  ...
  void step(){
    // snapshot for history & change detection
...
int main(int argc,char**argv){
  ...
  SDL_GLContext gl_ctx=SDL_GL_CreateContext(win); SDL_GL_MakeCurrent(win,gl_ctx); SDL_GL_SetSwapInterval(1);
  IMGUI_CHECKVERSION(); ImGui::CreateContext(); ImGuiIO&io=ImGui::GetIO(); (void)io; ImGui::StyleColorsDark();
  ImGui_ImplSDL2_InitForOpenGL(win,gl_ctx); ImGui_ImplOpenGL3_Init();

  CPU cpu; if(argc>1) cpu.load_bin(argv[1]);
  bool run=true, auto_run=false; char bin_path[256]="program.bin"; size_t mem_base=0; const int STEPS_PER_FRAME=500;

  while(run){ SDL_Event e; while(SDL_PollEvent(&e)){ ImGui_ImplSDL2_ProcessEvent(&e); if(e.type==SDL_QUIT) run=false; if(e.type==SDL_WINDOWEVENT && e.window.event==SDL_WINDOWEVENT_CLOSE && e.window.windowID==SDL_GetWindowID(win)) run=false; }
    ...
    // control panel
    ImGui::Begin("Control");
      if(ImGui::Button("Step")) cpu.step(); ImGui::SameLine();
      if(ImGui::Button("Step Back")) cpu.step_back(); ImGui::SameLine();
      if(ImGui::Button(auto_run?"Pause":"Run")) auto_run=!auto_run;
      ImGui::Separator();
      ImGui::InputText("Binary", bin_path, sizeof(bin_path)); ImGui::SameLine(); if(ImGui::Button("Load")){ cpu.load_bin(bin_path); }
      ImGui::Text("Cycle: %llu", (unsigned long long)cpu.cycles);
      ImGui::Text("PC: 0x%08X", cpu.pc);
      uint32_t ins=cpu.fetch(); ImGui::Text("Ins: 0x%08X  %s", ins, cpu.disasm(ins).c_str());
    ImGui::End();

    // register view
    ImGui::Begin("Registers"); if(ImGui::BeginTable("regtab",4,ImGuiTableFlags_Borders|ImGuiTableFlags_RowBg)){
      for(int i=0;i<32;++i){ ImGui::TableNextColumn(); if(cpu.reg_changed[i]) ImGui::PushStyleColor(ImGuiCol_Text,IM_COL32(255,80,80,255)); ImGui::Text("%s (x%d): 0x%08X / %d", REG_NAMES[i], i, cpu.x[i], (int32_t)cpu.x[i]); if(cpu.reg_changed[i]) ImGui::PopStyleColor(); }
      ImGui::EndTable(); }
    ImGui::End();

    // memory viewer
    ImGui::Begin("Memory"); ImGui::InputScalar("Base", ImGuiDataType_U64, &mem_base, NULL, NULL, "%08X", ImGuiInputTextFlags_CharsHexadecimal|ImGuiInputTextFlags_EnterReturnsTrue); size_t addr=mem_base&~0xF; const int ROWS=16; if(addr+ROWS*16<cpu.mem.size() && ImGui::BeginTable("mem",3,ImGuiTableFlags_RowBg|ImGuiTableFlags_BordersInnerV)){
      for(int r=0;r<ROWS;++r, addr+=16){ ImGui::TableNextRow(); ImGui::TableNextColumn(); ImGui::Text("0x%08X", (unsigned)addr); ImGui::TableNextColumn(); std::string hex,asc; for(int i=0;i<16;++i){ uint8_t b=cpu.mem[addr+i]; std::ostringstream hs; hs<<std::hex<<std::setw(2)<<std::setfill('0')<<(int)b; hex+=hs.str(); hex+=(i==7?"  ":" "); asc+= (b>=32 && b<127)?(char)b:'.'; } ImGui::Text("%s", hex.c_str()); ImGui::TableNextColumn(); ImGui::Text("%s", asc.c_str()); }
      ImGui::EndTable(); }
    ImGui::End();

    // history view
    ImGui::Begin("History"); if(ImGui::BeginTable("hist",3,ImGuiTableFlags_RowBg|ImGuiTableFlags_Borders)){
      ImGui::TableSetupColumn("Cycle"); ImGui::TableSetupColumn("PC"); ImGui::TableSetupColumn("Disasm");
      for(auto &t : cpu.trace){ ImGui::TableNextRow(); ImGui::TableNextColumn(); ImGui::Text("%llu", (unsigned long long)t.cyc); ImGui::TableNextColumn(); ImGui::Text("0x%08X", t.pc); ImGui::TableNextColumn(); ImGui::Text("%s", t.txt.c_str()); }
      ImGui::EndTable(); }
    ImGui::End();

    ImGui::Render(); glViewport(0,0,(int)io.DisplaySize.x,(int)io.DisplaySize.y); glClearColor(0.1f,0.1f,0.1f,1.0f); glClear(GL_COLOR_BUFFER_BIT); ImGui_ImplOpenGL3_RenderDrawData(ImGui::GetDrawData()); SDL_GL_SwapWindow(win);
  }

  // cleanup
  ImGui_ImplOpenGL3_Shutdown(); ImGui_ImplSDL2_Shutdown(); ImGui::DestroyContext(); SDL_GL_DeleteContext(gl_ctx); SDL_DestroyWindow(win); SDL_Quit(); return 0;
}
#endif // GUI
